...
export async function downloadScheduleImages(rounds: ExportScheduleRound[], onProgress?: (label: string) => void) {
  ...
  const pageCount = prepared.reduce((count, { fixtures, round }) => count + balancedSchedulePages(fixtures, schedulePosterCapacity(round.format)).length, 0);
  if (!pageCount) throw new Error("Нет матчей для скачивания. Выберите другой тур.");
  const files: DownloadFile[] = [];
  for (const { round, fixtures } of prepared) {
    const pages = balancedSchedulePages(fixtures, schedulePosterCapacity(round.format));
    ...
    for (const [index, page] of pages.entries()) {
      ...
      const canvas = await renderSchedulePoster(round, page, totalMatchCount, { page: index + 1, pageCount: pages.length });
      try {
        const blob = await canvasToBlob(canvas);
        if (!blob) throw new Error("Не удалось сохранить PNG. Попробуйте скачать один тур.");
        files.push({ name: `${fileSlug(round.sectionName || "schedule")}-${fileSlug(round.title)}-${files.length + 1}-${index + 1}.png`, blob });
      } finally {
        canvas.width = 0;
        canvas.height = 0;
      }
