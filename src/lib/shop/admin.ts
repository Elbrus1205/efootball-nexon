...
export async function getShopAdminDashboard(days = 30) {
  ...
  const [settings, categories, products, promotions, promoCodes, sellers, orders, revenue, completed, payments, refunds, disputes] = await db.$transaction([
    ...
    db.shopSeller.findMany({ where: { deletedAt: null }, include: { user: { select: { publicId: true, name: true, telegramUsername: true } }, _count: { select: { orders: { where: { status: { in: [ShopOrderStatus.ACCEPTED, ShopOrderStatus.IN_PROGRESS, ShopOrderStatus.SELLER_COMPLETED, ShopOrderStatus.WAITING_BUYER_CONFIRMATION, ShopOrderStatus.DISPUTE] } } } } } }, orderBy: { createdAt: "desc" } }),
