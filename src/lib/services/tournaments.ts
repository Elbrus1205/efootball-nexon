...
import { resolveStageGraphAssignments, topologicalStageOrder, transitionParticipantCount, type StageGraphBlueprint } from "@/lib/tournament-stage-graph";
...
import { resolvePodiumSeries } from "@/lib/tournaments/podium";
import {
...
function isAdvancedStageGraph(graph: StageGraphBlueprint) {
  ...
  return graph.mode === "VISUAL" || nonPlayoffCount > 1 || graph.superCup.enabled || graph.transitions.some((transition) => transition.result !== "RANK");
}

function getPersistedGraphStageId(stage: { id: string; settingsJson?: unknown }) {
  const settings = stage.settingsJson;
  if (!settings || typeof settings !== "object" || Array.isArray(settings)) return stage.id;
  const graphId = (settings as { graphId?: unknown }).graphId;
  return typeof graphId === "string" && graphId ? graphId : stage.id;
}

function getPersistedGraphStage(graph: StageGraphBlueprint | undefined, stage: { id: string; settingsJson?: unknown }) {
  if (!graph) return undefined;
  const graphId = getPersistedGraphStageId(stage);
  return graph.stages.find((node) => node.id === graphId);
}
...
function tournamentNotificationsEnabled(tournament: { notificationsEnabled?: boolean | null; isTest?: boolean }) {
  return !tournament.isTest && tournament.notificationsEnabled !== false;
}
...
async function reconcileBracketByes(bracketId: string, options: MatchResolutionOptions = {}) {
  ...
  for (const seriesMatches of Array.from(grouped.values())) {
    ...
    await advanceResolvedWinnerForMatch(match.id, byeWinner.userId, null, byeWinner.entryId, null, options);
  }
}
...
function roundUnitForStage(stageType: StageType) {
  return stageType === StageType.PLAYOFF || stageType === StageType.SUPER_CUP ? "Раунд" : "Тур";
}
...
async function createRoundRobinMatchesForEntries({
  ...
  const scheduleShape = resolveRoundRobinScheduleShape({ participantsCount: entries.length, roundsCount, matchesPerOpponent, roundsMode });
  const matchesPerPair = bestOfMatchesPerPair ?? scheduleShape.matchesPerPair;
  const totalTours = scheduleShape.totalTours;
  const maxMatchesPerPair = bestOfMatchesPerPair ?? (roundsMode === "series" ? matchesPerPair : requestedMatchesPerOpponent);
...
async function createPlayoffMatches({
  ...
  if (orderedEntries.length) {
    ...
    await reconcileBracketByes(bracketId, { deferTournamentSync });
  }
...
async function createAdvancedGraphStages(params: {
  ...
  const incomingStageIds = new Set(params.graph.transitions.map((transition) => transition.toStageId));
  for (const [index, node] of ordered.entries()) {
    const isPlayoff = node.type === "PLAYOFF" || node.type === "SUPERCUP";
    const stage = await db.tournamentStage.create({
      data: {
        tournamentId: params.tournamentId,
        name: node.name,
        type: node.type === "SUPERCUP" ? StageType.SUPER_CUP : isPlayoff ? StageType.PLAYOFF : node.type === "LEAGUE" ? StageType.LEAGUE : StageType.GROUP_STAGE,
        status: incomingStageIds.has(node.id) ? StageStatus.PENDING : getStageStatus(false, params.tournament.status),
        orderIndex: index + 1,
        ...
        settingsJson: {
          ...
          transitions: params.graph.transitions.filter((transition) => transition.fromStageId === node.id || transition.toStageId === node.id),
    ...
    if (!isPlayoff) {
      for (let division = 0; division < node.divisionsCount; division += 1) {
        const divisionSettings = node.divisions[division];
        await db.tournamentGroup.create({ data: { stageId: stage.id, name: divisionSettings?.name ?? (node.divisionsCount === 1 ? node.name : `${node.type === "LEAGUE" ? "Лига" : "Группа"} ${division + 1}`), orderIndex: division + 1, capacity: divisionSettings?.participantsCount ?? node.participantsPerDivision ?? undefined } });
      }
    } else {
      const transitions = params.graph.transitions.filter((transition) => transition.toStageId === node.id);
      const entries = transitions.reduce((sum, transition) => sum + transitionParticipantCount(params.graph, transition), 0);
      ...
      await db.playoffBracket.create({ data: { tournamentId: params.tournamentId, stageId: stage.id, type: node.playoffType ?? PlayoffType.SINGLE, size, legsCount: node.legsCount ?? 1, thirdPlaceMatch: node.thirdPlaceMatch ?? false, settingsJson: { mode: "custom-graph", graphId: node.id, transitions, bestOfWins: node.bestOfWins, bracketFill: node.bracketFill, penaltyRule: node.penaltyRule, seedingMethod: node.seedingMethod } } });
    }
    stages.push(stage);
  }
  return stages;
...
export async function assertTournamentEditAllowed(input: {
  ...
  const playoffStageIds = stages.filter((stage) => stage.type === StageType.PLAYOFF || stage.type === StageType.SUPER_CUP).map((stage) => stage.id);
  ...
  const drift = advancedGraph ? { openingDrift: false, playoffDrift: false } : findCustomStructureDrift(plan.expected, {
    opening: openingStage
      ...
      : null,
    playoffs: stages.filter((stage) => stage.type === StageType.PLAYOFF || stage.type === StageType.SUPER_CUP).map((stage) => {
...
export async function synchronizeTournamentAfterEdit(input: {
  ...
  const playoffStages = tournament.stages.filter((stage) => stage.type === StageType.PLAYOFF || stage.type === StageType.SUPER_CUP);
  const openingSettings = openingStage?.settingsJson && typeof openingStage.settingsJson === "object" && !Array.isArray(openingStage.settingsJson)
    ? openingStage.settingsJson as { mode?: unknown; matchesPerOpponent?: unknown }
    : null;
  ...
  if (advancedGraph) {
    ...
    if (graph) {
      for (const stage of tournament.stages) {
        ...
        } else if (stage.bracket && stage.bracket.matches.length === 0) {
          await db.playoffBracket.update({
            where: { id: stage.bracket.id },
            data: {
              type: node.playoffType ?? stage.bracket.type,
              size: node.bracketSize ?? stage.bracket.size,
              legsCount: node.legsCount ?? stage.bracket.legsCount,
              thirdPlaceMatch: node.thirdPlaceMatch ?? stage.bracket.thirdPlaceMatch,
              settingsJson: {
                mode: "custom-graph", graphId: node.id,
                transitions: graph.transitions.filter((transition) => transition.toStageId === node.id),
                bestOfWins: node.bestOfWins, bracketFill: node.bracketFill,
                penaltyRule: node.penaltyRule, seedingMethod: node.seedingMethod,
              } as Prisma.InputJsonValue,
            },
          });
        }
  ...
  if (rebuildPlayoffs) {
    await db.$transaction(async (tx) => {
      ...
      await tx.bracketSlot.deleteMany({ where: { bracket: { tournamentId: tournament.id } } });
      await tx.playoffBracket.deleteMany({ where: { tournamentId: tournament.id } });
      await tx.tournamentStage.deleteMany({ where: { tournamentId: tournament.id, type: { in: [StageType.PLAYOFF, StageType.SUPER_CUP] } } });
    });
    ...
    if (openingStage?.status === StageStatus.COMPLETED) {
      ...
      const refreshedPlayoffs = await db.tournamentStage.findMany({
        where: { tournamentId: tournament.id, type: { in: [StageType.PLAYOFF, StageType.SUPER_CUP] } },
...
export async function generateTournamentStages(tournamentId: string, options?: { regenerate?: boolean }) {
  ...
  if (options?.regenerate) {
    const protectedMatch = await db.match.findFirst({
      where: {
        tournamentId,
        OR: [
          { status: { in: [...PLAYED_MATCH_STATUSES] } },
    ...
    if (protectedMatch) {
      throw new TournamentEditConflictError("Нельзя пересоздать структуру: в турнире уже есть результаты, заявки на результат или исторические составы матчей.");
    }
...
export async function generateTournamentMatches(tournamentId: string) {
    ...
    include: {
      ...
      stages: {
        include: {
          ...
          entries: { include: { registration: true }, orderBy: { createdAt: "asc" } },
          bracket: true,
        },
        orderBy: { orderIndex: "asc" },
      },
  ...
  const advancedGraph = tournament.format === TournamentFormat.CUSTOM
    ? normalizeFormatBlueprint(tournament.formatBlueprintJson).stageGraph
    : undefined;
  if (advancedGraph && isAdvancedStageGraph(advancedGraph)) {
    const incomingStageIds = new Set(advancedGraph.transitions.map((transition) => transition.toStageId));
    const rootStages = tournament.stages.filter((stage) => {
      const settings = stage.settingsJson && typeof stage.settingsJson === "object" && !Array.isArray(stage.settingsJson)
        ? stage.settingsJson as { mode?: unknown; graphId?: unknown }
        : null;
      return settings?.mode === "custom-graph" && typeof settings.graphId === "string" && !incomingStageIds.has(settings.graphId);
    });
    const rootCapacities = rootStages.map((stage) => {
      const node = getPersistedGraphStage(advancedGraph, stage);
      if (!node) return tournament.participants.length;
      if (node.type === "PLAYOFF" || node.type === "SUPERCUP") return node.bracketSize ?? tournament.participants.length;
      return node.divisions.reduce((sum, division) => sum + (division.participantsCount ?? 0), 0) || tournament.participants.length;
    });
    const totalRootCapacity = rootCapacities.reduce((sum, capacity) => sum + capacity, 0);
    if (tournament.participants.length > totalRootCapacity) {
      throw new Error(`В стартовых этапах есть место только для ${totalRootCapacity} участников.`);
    }
    const rootAssignments = new Map<string, typeof tournament.participants>();
    let participantOffset = 0;
    for (const [rootIndex, stage] of rootStages.entries()) {
      const assigned = tournament.participants.slice(participantOffset, participantOffset + rootCapacities[rootIndex]!);
      rootAssignments.set(stage.id, assigned);
      participantOffset += rootCapacities[rootIndex]!;
    }
    for (const stage of rootStages.filter((item) => item.status === StageStatus.ACTIVE)) {
      const groups = stage.type === StageType.LEAGUE
        ? await db.tournamentGroup.findMany({ where: { stageId: stage.id }, orderBy: { orderIndex: "asc" } })
        : [];
      if (tournament.participantDistributionByLeague && stage.type === StageType.LEAGUE) {
        await assignParticipantsToLeagueDivisions(tournamentId, { id: stage.id, groups });
        continue;
      }
      const existingCount = await db.tournamentStageEntry.count({ where: { stageId: stage.id } });
      if (existingCount > 0) continue;
      const assigned = rootAssignments.get(stage.id) ?? [];
      if (stage.type === StageType.PLAYOFF || stage.type === StageType.SUPER_CUP) {
        await db.tournamentStageEntry.createMany({ data: assigned.map((entry, index) => ({ stageId: stage.id, registrationId: entry.id, resultJson: { seed: entry.seed ?? index + 1 } })) });
      } else {
        if (!groups.length) continue;
        const groupCapacities = groups.map((group) => Math.max(0, group.capacity ?? Math.ceil(assigned.length / groups.length)));
        if (assigned.length > groupCapacities.reduce((sum, capacity) => sum + capacity, 0)) {
          throw new Error(`В этапе «${stage.name}» недостаточно мест для участников.`);
        }
        const assignedCounts = groups.map(() => 0);
        await Promise.all(assigned.map(async (entry) => {
          const groupIndex = groups.findIndex((_, index) => assignedCounts[index]! < groupCapacities[index]!);
          const group = groups[groupIndex];
          if (!group) throw new Error(`Не удалось распределить участников этапа «${stage.name}».`);
          assignedCounts[groupIndex] = assignedCounts[groupIndex]! + 1;
          await db.tournamentStageEntry.create({ data: { stageId: stage.id, registrationId: entry.id, groupId: group.id } });
          await db.tournamentRegistration.update({ where: { id: entry.id }, data: { groupId: group.id } });
        }));
      }
    }
  }
  ...
  for (const stage of tournament.stages) {
    ...
    if (stage.type === StageType.LEAGUE) {
      if (stageSettings?.mode === "custom-graph" && stage.groups.length) {
        if (tournament.participantDistributionByLeague && stage.orderIndex === 1) {
          await assignParticipantsToLeagueDivisions(tournamentId, stage);
          const refreshedGroups = await db.tournamentGroup.findMany({ where: { stageId: stage.id }, orderBy: { orderIndex: "asc" } });
          for (const group of refreshedGroups) {
            const currentEntries = await db.tournamentStageEntry.findMany({ where: { stageId: stage.id, groupId: group.id, registration: { status: ParticipantStatus.CONFIRMED } }, select: { registration: { select: { id: true, userId: true } } } });
        ...
        for (const group of stage.groups) {
          const currentEntries = await db.tournamentStageEntry.findMany({ where: { stageId: stage.id, groupId: group.id, registration: { status: ParticipantStatus.CONFIRMED } }, select: { registration: { select: { id: true, userId: true } } } });
          const currentMembers = currentEntries.map((entry) => entry.registration);
      ...
      if (existingCount > 0) continue;
      const stageEntries = stageSettings?.mode === "custom-graph"
        ? (await db.tournamentStageEntry.findMany({ where: { stageId: stage.id, registration: { status: ParticipantStatus.CONFIRMED } }, select: { registration: { select: { id: true, userId: true } } } })).map((entry) => entry.registration)
        : (stage.entries.length ? stage.entries.map((entry) => ({ id: entry.registration.id, userId: entry.registration.userId })) : tournament.participants.map((entry) => ({ id: entry.id, userId: entry.userId })));
      if (stageSettings?.mode === "custom-graph" && stageEntries.length < 2) continue;
    ...
    if (stage.type === StageType.GROUP_STAGE) {
      for (const group of stage.groups) {
        const currentMembers = stageSettings?.mode === "custom-graph"
          ? (await db.tournamentStageEntry.findMany({ where: { stageId: stage.id, groupId: group.id, registration: { status: ParticipantStatus.CONFIRMED } }, select: { registration: { select: { id: true, userId: true } } } })).map((entry) => entry.registration)
          : group.members;
        const members = currentMembers.map((entry) => ({ id: entry.id, userId: entry.userId }));
    ...
    if ((stage.type === StageType.PLAYOFF || stage.type === StageType.SUPER_CUP) && stage.bracket) {
      const stageSettings = stage.settingsJson && typeof stage.settingsJson === "object" && !Array.isArray(stage.settingsJson) ? stage.settingsJson as { mode?: unknown } : null;
      if (stageSettings?.mode === "custom-graph" && stage.status !== StageStatus.ACTIVE) continue;
      if (stageSettings?.mode === "custom-graph") {
        const existingCount = await db.match.count({ where: { bracketId: stage.bracket.id } });
        if (existingCount > 0) continue;
        const stageEntries = await db.tournamentStageEntry.findMany({ where: { stageId: stage.id, registration: { status: ParticipantStatus.CONFIRMED } }, include: { registration: { select: { id: true, userId: true, seed: true } } }, orderBy: { createdAt: "asc" } });
        const entries = stageEntries.map((entry, index) => {
          const result = entry.resultJson && typeof entry.resultJson === "object" && !Array.isArray(entry.resultJson) ? entry.resultJson : null;
          return { ...entry.registration, seed: typeof result?.toSlot === "number" ? result.toSlot : index + 1 };
        });
        if (entries.length < 2) continue;
        const graphNode = getPersistedGraphStage(advancedGraph, stage);
        await createPlayoffMatches({ tournamentId, stageId: stage.id, bracketId: stage.bracket.id, entries, type: stage.bracket.type, legsCount: stage.bracket.legsCount, thirdPlaceMatch: stage.bracket.thirdPlaceMatch, sizeOverride: stage.bracket.size, matchupFormat: (graphNode?.bestOfWins ?? 1) > 1 ? MatchupFormat.BEST_OF : tournament.matchupFormat, bestOfWins: graphNode?.bestOfWins ?? tournament.bestOfWins, deferTournamentSync: true });
        continue;
      }
...
export async function recalculateGroupStandings(tournamentId: string) {
  ...
  for (const group of groups) {
    // Registration.groupId tracks the current stage. Stage entries retain the
    // participants of earlier leagues after qualification into a new group.
    const members = Array.from(new Map([
      ...group.stageEntries.map((entry) => entry.registration),
      ...group.members,
    ...
    const visibleMemberIds = new Set(members.filter((member) => member.status === ParticipantStatus.CONFIRMED).map((member) => member.id));
...
export async function generatePlayoffFromGroups(tournamentId: string) {
  ...
  if (tournament.format === TournamentFormat.CUSTOM) {
    ...
    const playoffStages = tournament.stages.filter((stage) => (stage.type === StageType.PLAYOFF || stage.type === StageType.SUPER_CUP) && stage.bracket);
...
export async function startTournament(tournamentId: string) {
  ...
  const requiresGroupAssignments =
    (!advancedGraph && tournament.format === TournamentFormat.CUSTOM && !isCustomDirectPlayoff(tournament.format, tournament.formatBlueprintJson)) ||
  ...
  if (advancedGraph) {
    await db.tournamentStage.updateMany({ where: { tournamentId }, data: { status: StageStatus.PENDING } });
    const graph = normalizeFormatBlueprint(tournament.formatBlueprintJson).stageGraph;
    if (graph) {
      const incomingStageIds = new Set(graph.transitions.map((transition) => transition.toStageId));
      const rootStages = tournament.stages.filter((stage) => {
        const settings = stage.settingsJson && typeof stage.settingsJson === "object" && !Array.isArray(stage.settingsJson) ? stage.settingsJson as { graphId?: unknown } : null;
        return typeof settings?.graphId === "string" && !incomingStageIds.has(settings.graphId);
      });
      await Promise.all(rootStages.map((stage) => db.tournamentStage.update({ where: { id: stage.id }, data: { status: StageStatus.ACTIVE, startsAt: stage.startsAt ?? new Date() } })));
    }
  }
  ...
  if (!advancedGraph) {
    await db.tournamentStage.updateMany({ where: { tournamentId }, data: { status: StageStatus.PENDING } });
    const firstStage = await db.tournamentStage.findFirst({ where: { tournamentId }, orderBy: { orderIndex: "asc" } });
    if (firstStage) {
      const activatedAt = new Date();
      await db.tournamentStage.update({ where: { id: firstStage.id }, data: { status: StageStatus.ACTIVE, startsAt: firstStage.startsAt ?? activatedAt } });
    }
  }
  await applyTournamentAbsenceRatingPenalty(tournamentId);
...
async function assignParticipantsToLeagueDivisions(
  tournamentId: string,
  stage: { id: string; groups: Array<{ id: string; name: string; capacity: number | null; orderIndex: number }> },
) {
  const participants = await db.tournamentRegistration.findMany({
    where: { tournamentId, status: ParticipantStatus.CONFIRMED },
    select: { id: true, groupId: true, clubSlug: true, clubName: true },
    orderBy: [{ seed: "asc" }, { createdAt: "asc" }],
  });
  const clubs = await db.club.findMany({
    where: { slug: { in: participants.flatMap((participant) => participant.clubSlug ? [participant.clubSlug] : []) } },
    select: { slug: true, league: { select: { slug: true } } },
  });
  const leagueByClubSlug = new Map(clubs.map((club) => [club.slug, club.league?.slug ?? null]));
  const groups = stage.groups.map((group) => ({
    id: group.id,
    leagueSlug: group.name === "АПЛ" ? "premier-league" : group.name === "Ла Лига" ? "la-liga" : group.name === "Бундеслига" ? "bundesliga" : group.name === "Серия А" ? "serie-a" : group.name === "Лига 1" ? "ligue-1" : "",
    capacity: group.capacity ?? participants.length,
  }));
  const assignments = assignParticipantsByLeague(participants.map((participant) => ({ ...participant, clubLeagueSlug: participant.clubSlug ? leagueByClubSlug.get(participant.clubSlug) : null })), groups);
  const targetGroupByRegistrationId = new Map(assignments.map((assignment) => [assignment.participant.id, assignment.groupId]));
  const existingEntries = await db.tournamentStageEntry.findMany({ where: { stageId: stage.id }, select: { registrationId: true, groupId: true } });
  const assignmentsChanged = participants.some((participant) => participant.groupId !== targetGroupByRegistrationId.get(participant.id)) ||
    existingEntries.length !== assignments.length ||
    existingEntries.some((entry) => entry.groupId !== targetGroupByRegistrationId.get(entry.registrationId));
  ...
  if (protectedMatch) {
    throw new TournamentEditConflictError("Нельзя перераспределить лиги: в этапе уже есть результаты или исторические составы матчей.");
  }

  await Promise.all(assignments.map((assignment) => db.tournamentRegistration.update({ where: { id: assignment.participant.id }, data: { groupId: assignment.groupId, seed: assignment.seed } })));
  await db.tournamentStageEntry.deleteMany({ where: { stageId: stage.id } });
  await db.tournamentStageEntry.createMany({ data: assignments.map((assignment) => ({ stageId: stage.id, registrationId: assignment.participant.id, groupId: assignment.groupId, resultJson: { seed: assignment.seed } })) });
  await db.groupStanding.deleteMany({ where: { groupId: { in: stage.groups.map((group) => group.id) } } });
  await Promise.all(stage.groups.map((group) => ensureGroupStandings(group.id, assignments.filter((assignment) => assignment.groupId === group.id).map((assignment) => assignment.participant.id))));
}

function getGraphPlayoffResults(matches: Match[]) {
  const seriesResult = (bracket: string, thirdPlace = false) => {
    const candidates = matches.filter((match) => match.bracket === bracket && match.isThirdPlaceMatch === thirdPlace);
    const lastRound = Math.max(0, ...candidates.map((match) => match.round));
    const final = candidates.filter((match) => match.round === lastRound);
    return resolvePodiumSeries(final.map((match) => ({ ...match, lineupPlayers: [] })));
  };
  const upper = seriesResult("upper");
  const lower = seriesResult("lower");
  const final = upper;
  const third = seriesResult("upper", true);
  return [
    ...(final ? [
      { registrationId: final.winnerEntryId, result: "WINNER" as const },
      { registrationId: final.loserEntryId, result: "RUNNER_UP" as const },
    ] : []),
    ...(upper ? [{ registrationId: upper.loserEntryId, result: "UPPER_BRACKET_LOSER" as const }] : []),
    ...(lower ? [{ registrationId: lower.winnerEntryId, result: "LOWER_BRACKET_WINNER" as const }] : []),
    ...(third ? [{ registrationId: third.winnerEntryId, result: "THIRD_PLACE" as const }] : []),
  ];
}
...
async function advanceAdvancedGraphStage(tournamentId: string, tournament: { formatBlueprintJson: unknown; matchupFormat: MatchupFormat; bestOfWins: number }) {
  ...
  const stages = await db.tournamentStage.findMany({
    where: { tournamentId },
    include: {
      groups: { include: { standings: { orderBy: { rank: "asc" } } }, orderBy: { orderIndex: "asc" } },
  ...
  let changed = false;
  // Record completion before checking dependencies, including terminal stages
  // without outgoing transitions. Never treat an empty stage as played.
  for (const stage of stages) {
    const matches = stage.bracket?.matches.length ? stage.bracket.matches : stage.matches;
    if (stage.status !== StageStatus.ACTIVE || !matches.length || !matches.every((match) => TERMINAL_MATCH_STATUSES.has(match.status))) continue;
    await db.tournamentStage.update({ where: { id: stage.id }, data: { status: StageStatus.COMPLETED } });
    stage.status = StageStatus.COMPLETED;
    changed = true;
  }
  const stagesByGraphId = new Map(stages.map((stage) => [getPersistedGraphStageId(stage), stage]));
  for (const targetNode of topologicalStageOrder(graph)) {
    const target = stagesByGraphId.get(targetNode.id);
    if (!target || target.status === StageStatus.COMPLETED) continue;
    const incoming = graph.transitions.filter((transition) => transition.toStageId === targetNode.id);
    if (!incoming.length) continue;
    const sourceIds = [...new Set(incoming.map((transition) => transition.fromStageId))];
    if (!sourceIds.every((id) => {
      const source = stagesByGraphId.get(id);
      return source?.status === StageStatus.COMPLETED && source.matches.length > 0 && source.matches.every((match) => TERMINAL_MATCH_STATUSES.has(match.status));
    })) continue;
    // Populate a destination from ALL its sources before generating its matches.
    // In particular, a Super Cup cannot start with only one cup winner.
    const assignments = sourceIds.flatMap((sourceId) => {
      const source = stagesByGraphId.get(sourceId)!;
      const sourceNode = graph.stages.find((node) => node.id === sourceId);
      const standings = source.groups.flatMap((group) => group.standings.map((standing) => ({
        registrationId: standing.participantId, divisionIndex: group.orderIndex,
        divisionId: sourceNode?.divisions[group.orderIndex - 1]?.id ?? null, rank: standing.rank,
      })));
      return resolveStageGraphAssignments({ graph, fromStageId: sourceId, standings, playoffResults: getGraphPlayoffResults(source.bracket?.matches ?? []) });
    }).filter((assignment) => assignment.toStageId === targetNode.id);
    const uniqueAssignments = Array.from(new Map(assignments.map((assignment) => [assignment.registrationId, assignment])).values());
    if (uniqueAssignments.length < 2) continue;
    const existingEntries = await db.tournamentStageEntry.findMany({ where: { stageId: target.id }, select: { sourceTransitionId: true, registrationId: true } });
    const existingKeys = new Set(existingEntries.map((entry) => `${entry.sourceTransitionId}:${entry.registrationId}`));
    const pendingAssignments = uniqueAssignments.filter((assignment) => !existingKeys.has(`${assignment.sourceTransitionId}:${assignment.registrationId}`));
    if (target.status === StageStatus.ACTIVE && target.matches.length) continue;
    if (pendingAssignments.length || target.status !== StageStatus.ACTIVE) {
      const entryData = pendingAssignments.map((assignment) => {
        const divisionIndex = assignment.toDivisionId
          ? targetNode.divisions.findIndex((division) => division.id === assignment.toDivisionId) + 1
          : assignment.toDivisionIndex;
        const group = target.groups.find((item) => item.orderIndex === divisionIndex);
        if (target.groups.length && !group) throw new Error(`Не найдена целевая лига этапа «${target.name}». Проверьте переходы в настройках турнира.`);
        return {
          stageId: target.id, registrationId: assignment.registrationId, groupId: group?.id ?? null,
          sourceTransitionId: assignment.sourceTransitionId,
          resultJson: { targetBracket: assignment.targetBracket, toSlot: assignment.toSlot },
        };
      });
      await db.$transaction(async (tx) => {
        const existingIds = new Set(existingEntries.map((entry) => entry.registrationId));
        await tx.tournamentStageEntry.createMany({ data: entryData.filter((entry) => !existingIds.has(entry.registrationId)), skipDuplicates: true });
        for (const entry of entryData.filter((item) => existingIds.has(item.registrationId))) {
          await tx.tournamentStageEntry.upsert({
            where: { stageId_registrationId: { stageId: target.id, registrationId: entry.registrationId } },
            update: entry, create: entry,
          });
        }
        for (const groupId of new Set(entryData.map((entry) => entry.groupId))) {
          const registrationIds = entryData.filter((entry) => entry.groupId === groupId).map((entry) => entry.registrationId);
          await tx.tournamentRegistration.updateMany({ where: { id: { in: registrationIds } }, data: { groupId } });
          if (groupId) await tx.groupStanding.createMany({
            data: registrationIds.map((participantId) => ({ groupId, participantId })), skipDuplicates: true,
          });
        }
        await tx.tournamentStage.update({ where: { id: target.id }, data: { status: StageStatus.ACTIVE, startsAt: target.startsAt ?? new Date() } });
      }, { timeout: 60_000 });
      target.status = StageStatus.ACTIVE;
      changed = true;
    }
  }
  if (changed) invalidateTournamentAll(tournamentId);
  return changed;
}

export async function syncTournamentLifecycleStatus(tournamentId: string, options: { notify?: boolean } = {}) {
  const tournament = await db.tournament.findUnique({
    where: { id: tournamentId },
    include: {
      participants: {
  ...
  const advancedGraph = tournament.format === TournamentFormat.CUSTOM && isAdvancedStageGraph(normalizeFormatBlueprint(tournament.formatBlueprintJson).stageGraph!);
  let allGraphStagesCompleted = false;
  if (advancedGraph) {
    const advancedChanged = await advanceAdvancedGraphStage(tournamentId, tournament);
    const currentStages = await db.tournamentStage.findMany({
      where: { tournamentId },
      select: { id: true, settingsJson: true, status: true, _count: { select: { matches: true, entries: true } } },
    });
    const graph = normalizeFormatBlueprint(tournament.formatBlueprintJson).stageGraph!;
    allGraphStagesCompleted = graph.stages.every((node) => currentStages.some((stage) => getPersistedGraphStageId(stage) === node.id && stage.status === StageStatus.COMPLETED));
    const needsMatches = currentStages.some((stage) => stage.status === StageStatus.ACTIVE && stage._count.entries >= 2 && stage._count.matches === 0);
    if (advancedChanged || needsMatches) {
  ...
  if (tournament.format === TournamentFormat.CUSTOM && !advancedGraph) {
    const leagueStage = tournament.stages.find((stage) => stage.type === StageType.GROUP_STAGE);
    ...
    if (leagueMatchesCompleted && playoffStages.length && !hasPlayoffMatches) {
      ...
      if (options.notify !== false) await notifyActiveTournamentRoundsStarted(tournamentId);
  ...
  const nextStatus =
    advancedGraph && hasMatches && !allGraphStagesCompleted
      ? TournamentStatus.IN_PROGRESS
      : allMatchesCompleted && (!advancedGraph || allGraphStagesCompleted)
  ...
  if (options.notify !== false && nextStatus === TournamentStatus.AWAITING_START && tournamentNotificationsEnabled(tournament)) {
    await createNotificationsForUsers({
  ...
  if (options.notify !== false && nextStatus === TournamentStatus.COMPLETED) {
