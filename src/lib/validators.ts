...
export const resultSubmissionSchema = z.object({
  ...
  player1PenaltyScore: z.coerce.number().min(0).max(99).optional(),
  player2PenaltyScore: z.coerce.number().min(0).max(99).optional(),
...
export const participantManageSchema = z.object({
  action: z.enum(["add", "remove", "replace", "addMember", "replaceMember", "seed", "status", "setInactive", "setActive"]),
  userId: z.string().optional(),
  registrationId: z.string().optional(),
  memberId: z.string().optional(),
  replacementUserId: z.string().optional(),
  clubSlug: z.string().trim().optional().or(z.literal("")),
