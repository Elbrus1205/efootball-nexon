...
export function ShopProductForm({ categories, initial }: { categories: CategoryOption[]; initial?: ProductInitial }) {
  ...
  return <form action="/api/admin/shop" method="post" className={`${styles.adminCard} ${styles.adminForm}`}>
    ...
    <div className={styles.switchGrid}>
      <label className={styles.checkLabel}><input type="checkbox" name="quantityEnabled" value="true" defaultChecked={initial?.quantityEnabled} /> Разрешить выбор количества<span className={styles.helper}>Если выключено, покупатель оформляет ровно 1 единицу.</span></label>
