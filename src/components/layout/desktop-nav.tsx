...
export function DesktopNav({ links }: { links: DesktopNavLink[] }) {
  ...
  return (
    <nav aria-label="Основная навигация" className="hidden items-center gap-0.5 rounded-full border border-white/[0.07] bg-white/[0.025] p-1 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] lg:flex">
      {links.map((link) => {
        ...
        return (
          <Link
            ...
            className={cn(
              "group relative isolate overflow-hidden rounded-full px-3 py-2 text-[12px] font-bold leading-none text-zinc-300 transition-[color,background-color,box-shadow,transform] duration-300 ease-out hover:-translate-y-0.5 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#21F1A8]/60 xl:px-3.5",
