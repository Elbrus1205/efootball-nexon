...
import { ShopProductFieldType, ShopProductType, ShopStockMode } from "@prisma/client";
...
export async function POST(request: Request) {
  ...
  const manageActions = new Set(["saveSettings", "createCategory", "createProduct", "toggleProduct", "createField", "addSeller", "toggleSeller", "assignSellerProduct", "createPromotion", "createPromoCode", "moderateReview"]);
    ...
    if (action === "saveSettings") {
      ...
      const product = await db.shopProduct.create({
        data: {
          ...
          variants: { create: { sku: text(form, "sku").toUpperCase(), name: text(form, "variantName") || "Стандартный", priceMinor, stockMode: form.get("unlimited") === "true" ? ShopStockMode.UNLIMITED : ShopStockMode.FINITE, stockQuantity: integer(form, "stockQuantity"), isDefault: true, createdById: session.user.id } },
        },
      });
      ...
      await db.shopSeller.upsert({ where: { userId: user.id }, create: { userId: user.id, maxActiveOrders: integer(form, "maxActiveOrders", 3), commissionBps: integer(form, "commissionBps", 3000) }, update: { isActive: true, deletedAt: null, maxActiveOrders: integer(form, "maxActiveOrders", 3), commissionBps: integer(form, "commissionBps", 3000) } });
      ...
      const promotion = await db.shopPromotion.create({
        data: {
          ...
          endsAt: new Date(text(form, "endsAt")),
