...
import { Coins, Handshake, Link2, Percent, Search, Trash2, Users } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <CardDescription>Партнёрская программа с промокодами, реферальными ссылками и расчётом профита.</CardDescription>
          </div>
        </CardHeader>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Найдите пользователя сайта, затем выдайте ему промокод и реферальную ссылку.</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <form action="/api/admin/coins/partners" method="post" className="grid gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <label className="space-y-2">
              ...
              <input name="promoCode" required placeholder="NEXON20" className="h-11 w-full rounded-xl border border-white/10 bg-black/40 px-4 text-sm text-white uppercase" />
            </label>
            <label className="space-y-2">
              <span className="text-sm text-zinc-300">Реферальная ссылка / slug</span>
              <input name="referralSlug" required placeholder="partner-name или /ref/partner-name" className="h-11 w-full rounded-xl border border-white/10 bg-black/40 px-4 text-sm text-white" />
            </label>
            <label className="space-y-2">
      ...
      <div className="grid gap-4">
        {partners.map((partner) => {
          ...
          const promoUses = partner.purchases.filter((purchase) => purchase.source === "PROMO_CODE").length;
          const referralPurchases = partner.purchases.filter((purchase) => purchase.source === "REFERRAL_LINK").length;
          const referralLink = formatReferralLink(baseUrl, partner.referralSlug);
            ...
            <Card key={partner.id} className="overflow-hidden">
                ...
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs text-zinc-400">
                    ...
                    <span className="rounded-full border border-white/10 bg-black/20 px-3 py-1">
                      Активации: {promoUses}/{partner.activationLimit || "∞"}
                    </span>
                  </div>
                  <div className="mt-3 flex items-center gap-2 break-all rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-sm text-zinc-300">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {[
                  ...
                  ["Покупки по ссылке", referralPurchases],
                  ["Рефералы", partner._count.referrals],
                  ["Покупки всего", partner._count.purchases],
                    ...
                    {partner.purchases.slice(0, 8).map((purchase) => (
                      <div key={purchase.id} className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-sm">
                        ...
                        <div className="mt-1 text-xs text-zinc-500">
                          {purchase.source === "PROMO_CODE" ? "Промокод" : "Реф. ссылка"} • оборот {formatMoney(purchase.paidAmountKopecks)} • профит {formatMoney(purchase.profitKopecks)} • партнёру {formatMoney(purchase.partnerEarningKopecks)}
                        </div>
                      </div>
                    ))}
