...
import { BarChart3, Boxes, ChevronDown, CircleDollarSign, PackageCheck, Settings2, ShieldCheck, ShoppingBag, Tag, TicketPercent, Trash2, Users } from "lucide-react";
  ...
  return <div className="space-y-6">
    ...
    {canManage ? <section>
      ...
      <div className={styles.editorStack}>
        ...
        <Editor title="Добавить исполнителя" caption="Исполнитель автоматически получает любые товары">
          <form action="/api/admin/shop" method="post" className={styles.adminForm}>
            ...
            <button className={styles.button}><Users /> Добавить исполнителя</button>
          </form>
        </Editor>
    ...
    {canManage ? <section>
      ...
      <div className={styles.editorStack}>{data.categories.map((category) => <Editor key={category.id} title={category.name} caption={`/${category.slug} · ${category.isActive ? "показывается" : "скрыта"}`}>
        ...
        <form action="/api/admin/shop" method="post" className={styles.dangerForm}><input type="hidden" name="_action" value="deleteCategory" /><input type="hidden" name="id" value={category.id} /><label className={styles.fieldLabel}>Введите УДАЛИТЬ<input className={styles.input} name="confirmation" required autoComplete="off" /></label><button className={styles.buttonDanger}><Trash2 /> Удалить категорию</button></form>
      </Editor>)}</div>
    </section> : null}
    ...
    <section>
      ...
      {data.products.length ? <div className={styles.editorStack}>{data.products.map((product) => {
        ...
        return <Editor key={product.id} title={product.title} caption={`${product.category.name} · ${variant ? formatShopMoney(variant.priceMinor) : "нет варианта"} · ${product.isActive ? "опубликован" : "черновик"}`}>
          {canManage && variant ? <>
            <ShopProductForm categories={categoryOptions} initial={{ id: product.id, variantId: variant.id, categoryId: product.categoryId, type: product.type, title: product.title, slug: product.slug, sku: variant.sku, variantName: variant.name, price: String(variant.priceMinor / 100), stockMode: variant.stockMode, stockQuantity: variant.stockQuantity, estimatedMinutes: variant.estimatedMinutes ?? product.estimatedMinutes, maxPerOrder: variant.maxPerOrder, quantityEnabled: variant.quantityEnabled, imageUrl: product.images[0]?.url ?? "", isActive: product.isActive, isFeatured: product.isFeatured, isPopular: product.isPopular }} />
            <form action="/api/admin/shop" method="post" className={styles.dangerForm}><input type="hidden" name="_action" value="deleteProduct" /><input type="hidden" name="id" value={product.id} /><label className={styles.fieldLabel}>Введите УДАЛИТЬ<input className={styles.input} name="confirmation" required autoComplete="off" /></label><button className={styles.buttonDanger}><Trash2 /> Удалить товар</button></form>
    ...
    <section><div className={styles.sectionHead}><div><p className={styles.eyebrow}>Команда магазина</p><h2 className={styles.sectionTitle}>Исполнители</h2></div></div><div className={styles.editorStack}>{data.sellers.map((seller) => <Editor key={seller.id} title={seller.user.name ?? seller.user.publicId} caption={`@${seller.user.telegramUsername ?? "без Telegram"} · ${seller._count.orders} незавершённых заказов`}><div className={styles.adminForm}>{canManage ? <form action="/api/admin/shop" method="post" className={styles.dangerForm}><input type="hidden" name="_action" value="removeSeller" /><input type="hidden" name="id" value={seller.id} /><label className={styles.fieldLabel}>Введите УДАЛИТЬ<input className={styles.input} name="confirmation" required autoComplete="off" /></label><button className={styles.buttonDanger}><Trash2 /> Удалить исполнителя</button></form> : null}</div></Editor>)}</div></section>
