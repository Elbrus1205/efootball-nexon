...
Все текстовые файлы проекта сохраняйте в UTF-8. Команда `npm run check:encoding` проверяет исходники, конфигурацию, документацию и текстовые ресурсы: некорректные байты UTF-8, NUL и символы замены U+FFFD приводят к ошибке. Проверка автоматически запускается в `npm run build` и `npm run verify`, работает без Git, в том числе внутри Docker. Зависимости, результаты сборки, резервные копии и бинарные ресурсы исключены.

В Windows PowerShell не используйте `Set-Content` или перенаправление вывода с кодировкой по умолчанию для исходников: это может записать Windows-1251 или UTF-16. При программной записи задавайте UTF-8 явно. Обычные вопросительные знаки и уже искажённые, но допустимые UTF-8 строки требуют просмотра diff; проверка кодировки не восстанавливает их автоматически.
...
2. Задайте `SHOP_ENC_KEY` (32 случайных байта в base64 или 64 hex-символа).
